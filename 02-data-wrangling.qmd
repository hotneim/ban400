...
First, we warm up with a first few functions from dplyr. Note how the syntax is similar to how we use the pipe \`\|\>. You can download the data set here: [data-geilo.xlsx](data-geilo.xlsx).
...
Finally, we will join data frames. See the [R4DS-book](data%20wrangling%20cheat%20sheet). Joins are necessary when we want to analyse two or more data sets jointly. In our simple example, we have two data sets: one with transactions, and one with customers:
